...
int crypt_and_hash(int argc, char *argv[])
{
    ...
    char *mmode = argv[4];
    char *alg;
    if (!strcmp(mmode, "aes-cbc"))
        alg = "AES-128-CBC";
    else if (!strcmp(mmode, "aes-ctr"))
        alg = "AES-128-CTR";
    else if (!strcmp(mmode, "aes-gcm"))
        alg = "AES-128-GCM";
    else if (!strcmp(mmode, "camellia-cbc"))
        alg = "CAMELLIA-128-CBC";
    else if (!strcmp(mmode, "aria-cbc"))
        alg = "ARIA-128-CBC";
    else if (!strcmp(mmode, "des-cbc"))
        alg = "DES-EDE3-CBC";
    else if (!strcmp(mmode, "chacha-poly1305"))
    ...
    if (cipher_info == NULL)
    {
        printf("Failed cipher info from string\n");
        goto exit;
    }
    if ((ret = mbedtls_cipher_setup(&cipher_ctx, cipher_info)) != 0)
    {
        printf("Failed cipher setup\n");
        goto exit;
    }
    ...
    if (md_info == NULL)
    {
        printf("Failed md info from string\n");
        goto exit;
    }

    if (mbedtls_md_setup(&md_ctx, md_info, 1) != 0)
    {
        printf("Failed md setup\n");
        goto exit;
    }
    ...
    if (mode == MODE_ENCRYPT)
    {
        ...
        {
            ilen = ((unsigned int)filesize - offset > mbedtls_cipher_get_block_size(&cipher_ctx)) ? mbedtls_cipher_get_block_size(&cipher_ctx) : (unsigned int)(filesize - offset);
