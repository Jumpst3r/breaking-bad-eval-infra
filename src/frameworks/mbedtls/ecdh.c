...
int ecdh(char* ec, char *rand, size_t r_len)
{
    int ret = 1;
    ...
    size_t srv_olen;
    size_t cli_olen;
    unsigned char secret_cli[128] = { 0 };
    unsigned char secret_srv[128] = { 0 };
    ...
    mbedtls_ecp_group_id ec_grp = MBEDTLS_ECP_DP_NONE;
    if (strstr(ec, "25519") != NULL)
        ec_grp = MBEDTLS_ECP_DP_CURVE25519;
    else if (strstr(ec, "256") != NULL)
    ...
    ret = mbedtls_ecdh_setup(&ctx_cli, ec_grp);
    if (ret != 0) {
    ...
    if (ret != 0) {
        mbedtls_printf(" failed\n  ! mbedtls_ecdh_make_params returned %d\n",
                       ret);
        mbedtls_printf("  ! curve supports ecdh: %d\n", mbedtls_ecdh_can_do(ec_grp));
    ...
    ret = mbedtls_ecdh_make_public(&ctx_srv, &srv_olen, srv_to_cli,
                                   ...
                                   mbedtls_ctr_drbg_random, &ctr_drbg);
    if (ret != 0) {
        mbedtls_printf(" failed\n  ! mbedtls_ecdh_make_public returned %x\n",
                       ret);
        goto exit;
    }
    ...
    ret = mbedtls_ecdh_read_public(&ctx_cli, srv_to_cli,
                                   srv_olen);
    if (ret != 0) {
        mbedtls_printf(" failed\n  ! mbedtls_ecdh_read_public returned -0x%x\n",
                       -ret);
        goto exit;
    }
    ...
    ret = mbedtls_ecdh_calc_secret(&ctx_cli, &cli_olen, secret_cli,
                                   ...
                                   mbedtls_ctr_drbg_random, &ctr_drbg);
    if (ret != 0) {
        mbedtls_printf(" failed\n  ! mbedtls_ecdh_calc_secret returned -0x%x\n",
                       -ret);
        goto exit;
    }
    ...
    mbedtls_printf("ecdh successful");
